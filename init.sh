...
if [ ! -s /etc/supervisor/conf.d/damon.conf ]; then
  ...
  [ -n "$GH_PROXY" ] && wget --server-response --quiet --output-document=/dev/null --no-check-certificate --tries=2 --timeout=3 https://raw.githubusercontent.com/dsadsadsss/nodejs-Nezha-server/main/README.md >/dev/null 2>&1 && unset GH_PROXY
  ...
  if [ "$REVERSE_PROXY_MODE" = 'grpcwebproxy' ]; then
    wget -c ${GH_PROXY}https://github.com/dsadsadsss/nodejs-Nezha-server/releases/download/grpcwebproxy/grpcwebproxy-linux-$ARCH.tar.gz -qO- | tar xz -C $WORK_DIR
...
EOF
  ...
  wget -P ${WORK_DIR}/data/ ${GH_PROXY}https://github.com/dsadsadsss/nodejs-Nezha-server/raw/main/sqlite.db
...
EOF
  ...
  wget -qO- ${GH_PROXY}https://raw.githubusercontent.com/dsadsadsss/nodejs-Nezha-server/main/template/backup.sh | sed '1,/^########/d' >> $WORK_DIR/backup.sh
...
EOF
    ...
    wget -qO- ${GH_PROXY}https://raw.githubusercontent.com/dsadsadsss/nodejs-Nezha-server/main/template/restore.sh | sed '1,/^########/d' >> $WORK_DIR/restore.sh
  fi
...
EOF
  ...
  wget -qO- ${GH_PROXY}https://raw.githubusercontent.com/dsadsadsss/nodejs-Nezha-server/main/template/renew.sh | sed '1,/^########/d' >> $WORK_DIR/renew.sh
