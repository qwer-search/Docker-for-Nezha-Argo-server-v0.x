...
if [ -e $TEMP_DIR/backup.tar.gz ]; then
  ...
  if [ "$IS_DOCKER" = 1 ]; then
    ...
    LOCAL_DATE=$(sqlite3 ./data/sqlite.db "SELECT created_at FROM servers WHERE name LIKE '%local%' COLLATE NOCASE LIMIT 1;") 
    ...
    [ "$DB_TOKEN" != "$LOCAL_TOKEN" ] && sqlite3 ./data/sqlite.db "UPDATE servers SET secret='${LOCAL_TOKEN}' WHERE created_at='${LOCAL_DATE}';"
